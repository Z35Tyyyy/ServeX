    ...
    return (
        <div style={{ padding: 32 }}>
            ...
            {showQR && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 1000 }}>
                    <div style={{ width: '100%', maxWidth: 360, background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', borderRadius: 16 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottom: '1px solid var(--color-border)' }}><h2>Table {showQR.tableNumber} QR</h2><button onClick={() => setShowQR(null)} style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-muted)' }}><X size={20} /></button></div>
                        <div style={{ padding: 24, textAlign: 'center' }}><img src={showQR.qrCodeUrl} alt="QR Code" style={{ maxWidth: 200, margin: '0 auto', borderRadius: 8, background: 'white', padding: 16 }} /><p style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 12, wordBreak: 'break-all' }}>{showQR.qrCodeData}</p></div>
